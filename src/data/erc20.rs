...
impl Convert for Erc20Token {
    ...
    async fn get_token_oracle_price(
        ...
        if self.symbol == "BTC" {
            ...
            let scale = BigDecimal::from_u64(10_u64.pow(8)).unwrap();

            let btc_price_oracle = chainlink_btc_oracle.latest_answer().call().await?;
            let btc_price_oracle = BigDecimal::from_str(&btc_price_oracle.to_string())?;
            let btc_price_oracle = btc_price_oracle / scale;
